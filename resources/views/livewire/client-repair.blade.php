<div>
    ...
    <form action="{{ route('client-repairs.store') }}" method="POST" class="form-wrap client-repair-form">
        @csrf
        <!-- Client Name -->
        <input type="text" class="form-control client-input" name="client_name" placeholder="Your Full Name" value="{{ old('client_name') }}">
        @error('client_name') <span class="text-danger">{{ $message }}</span> @enderror

        <!-- Contact Number -->
        <input type="text" class="form-control client-input" name="contact_number" placeholder="Your Contact Number" value="{{ old('contact_number') }}">
        ...
        <!-- Problem Description -->
        <textarea class="form-control client-req" name="problem_description" placeholder="Tell me your printer problem" cols="30" rows="5">{{ old('problem_description') }}</textarea>
